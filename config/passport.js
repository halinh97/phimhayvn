...
// ADMIN
passport.use('local-login_admin', new LocalStrategy({
    // by default, local strategy uses username and password, we will override with email
    usernameField : 'admin',
    passwordField : 'password',
    passReqToCallback : true // allows us to pass in the req from our route (lets us check if a user is logged in or not)
...
function(req, admin, password, done) {
  User.findOne({ where: { localadmin: admin }})
    .then(function(user) {
      if (!user) {
        done(null, false, req.flash('loginMessage', 'Bạn không phải admin '));
      } else if (!user.validPassword(password)) {
        done(null, false, req.flash('loginMessage', 'Mật khẩu không đúng'));
      } else {
        done(null, user);
      }
    })
    ...
    /////////////////////////// adim singup
    passport.use('local-signup_admin', new LocalStrategy({
        // by default, local strategy uses username and password, we will override with email
        usernameField : 'admin',
        passwordField : 'password',
        passReqToCallback : true // allows us to pass in the req from our route (lets us check if a user is logged in or not)
    ...
    function(req, admin, password, done) {
		...
		User.findOne({ where: { localadmin: admin }})
			.then(function(existingUser) {
				...
				// check to see if there's already a user with that email
				if (existingUser)
					return done(null, false, req.flash('loginMessage', 'That admin is already taken.'));
				...
				//  We're not logged in, so we're creating a brand new user.
					...
					// create the user
					var newUser = User.build ({localadmin: admin, localpassword: User.generateHash(password)});
					newUser.save().then(function() {done (null, newUser);}).catch(function(err) { done(null, false, req.flash('loginMessage', err));});
				}
			})
			.catch(function (e) {
